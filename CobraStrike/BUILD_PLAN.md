...
| `COBRA_ENABLE_PROXY` | `0` | Gate for Tor routing (`tor=1` on recon/web/creds/c2_gs_* tools). Own knob — exit-node egress is an **independent axis** from `COBRA_ALLOW_INTERNET`. Default off |
| `COBRA_PROXY` | `socks5h://127.0.0.1:9050` | SOCKS URL for the Tor/proxy listener. recon/web/creds route through it via a generated proxychains4 conf; `c2_gs_*` tools use gs-netcat's native `-T` (`GSOCKET_TOR`). Point at a non-standard tor port or another SOCKS proxy. A `.onion` target without `tor=1` is refused pre-spawn |
...
- **Brain / memory:** `brain_write` (full-document replace), `brain_append` (dated note; Lessons Learned), `brain_read` (ungated read of the CURRENT brain from disk — the startup context snapshot goes stale after every write; the model re-reads before planning and before a brain_write — the `mission_read` precedent), `mission_begin` (seed a mission file's objective into the brain's `## Mission` section — path-contained read like `mission_read`; the `cobra mission` CLI calls it automatically so the mission always lands in the brain, not just the prompt)
...
> **The brain loop is enforced, not voluntary (Phase 9, 2026-08-30).** The brain
> is the engagement's ONLY cross-run memory (server state is in-memory per
> process) and the dedup record ("Attempted & Failed" is what stops repeated
> scans). Before Phase 9 the loop was doctrine-only — the model could run a
> whole engagement without a single brain_write, and the operator couldn't see
> it (tool results hid behind `--verbose`). Now: `cobra mission <file>` seeds
> the Mission section deterministically via `mission_begin`; `brain_read` lets
> the agent re-read the current brain mid-session (the startup snapshot goes
> stale); the agent loop injects a `[checkpoint]` nudge after every 8 non-brain
> tool calls; the end-of-run summary warns loudly if the brain was never
> touched; and brain-write confirmations + tool errors are always visible.
...
- **Phase 8 — mission flow fix ✅ (2026-08-28)** — First ISO test found `cobra mission <file>` silently degraded to a `run` task with the literal words "mission <file>": the OS launcher prepends `--server-command/--server-args` BEFORE the operator's args, but `parseArgs` only recognized a subcommand at argv[0] — so the mission file was never read or injected and the agent hit the gated `shell_run` trying to `cat` it. Fixes: (a) client parser is now flags-first tolerant (first bare token naming a known command is the command wherever it lands; bool flags never eat the next token; `--flag=value` supported), plus a `mission` subcommand UX guard (empty file refused, "Mission file loaded (N bytes)" confirmation) and a runTask hint when a task names a `.mission.md` that was never loaded; (b) new ungated read-only `mission_read` tool (`tools/mission.ts`, path-contained via the shared `resolveContained` guard) because MCP *resources* are not model-callable mid-session in the agent loop — `cobra://missions/{file}` alone could never be read by the agent; (c) opshelp + `authorized-engagement` prompt now point at `mission_read` and stop telling the agent to read `cobra://missions/{file}` directly. Harness: scenario U + A9d registration, EXPECTED_TOOLS=50.
- **Phase 9 — enforced brain loop ✅ (2026-08-30)** — ISO ops found the agent re-running identical scans with an empty brain: the write side existed (`brain_write`/`brain_append`) but the loop was *model-voluntary* — no seeding, no mid-run re-read, no checkpoints, no verification, and tool results hid behind `--verbose`. Fixes: (a) new `tools/memory.ts` — `brain_read` (ungated read of the CURRENT brain from disk; the startup context snapshot goes stale after writes — the `mission_read` precedent) and `mission_begin` (path-contained read of a mission file + deterministic seed into the brain's `## Mission` section, splice-aware); (b) the client's `mission` subcommand calls `mission_begin` automatically before the agent starts (absolute paths outside the missions dir are refused there → prompt-only fallback), so the mission always lands in the brain; (c) the agent loop injects an in-band `[checkpoint]` nudge after every 8 non-brain tool calls (cap 4/run) telling the model to `brain_read` → `brain_write`, and counts brain writes for the end-of-run summary; (d) `runTask` warns loudly when a run ends with zero brain writes (distinguishing "mission seeded, no findings recorded" from "brain never touched"); (e) console visibility — `TOOL ERROR`/`TOOL ARG PARSE ERROR` results and brain-write confirmations are always shown (no `--verbose` needed), and tool-call arg echo is truncated to 160 chars so a full-document `brain_write` no longer dumps the brain to the terminal. Harness: scenario V (write/append/read round-trip on disk, empty-write refused, mission_begin seed + traversal refused) + A-count bump, EXPECTED_TOOLS=52.
- **Phase 10 — `creds_brute` session-managed ✅ (2026-08-31)** — ISO test of `creds_brute host=… service=ssh user=debian passlist=rockyou` failed with `TOOL ERROR: MCP error -32001: Request timed out` (cobra-client has no `callTool` timeout → the MCP SDK default of 60s fires `RequestTimeout`; hydra against a real service runs for hours). `creds_brute` no longer blocks on `runToLoot`: it starts hydra via `startSession("brute", …)` and returns a session id immediately — the agent polls with `session_output` (a hit prints `login: <user> password: <pw>`) and stops with `session_kill`, exactly like `listen_start`/`tunnel_socks_start`. Server-only fix; the client needs no change because the request now returns in ms. `SessionInfo.kind` gains `"brute"`; opshelp + the credential-attack prompt + playbook now say to poll the session. `creds_crack_john`/`hashcat` stay `runToLoot` (bounded by input, usually under a minute; still capped at 1h server-side). Harness: scenario W (creds_brute registered + returns a `brute-` session id, never blocks), EXPECTED_TOOLS=52 (no tool-count change).
