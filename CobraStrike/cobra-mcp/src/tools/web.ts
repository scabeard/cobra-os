...
export function registerWebTools(server: McpServer): void {
  server.tool(
    ...
    async ({ url, wordlist }) => {
      ...
      if (ffuf) {
        const r = await runToLoot("web_dir_brute", [ffuf, "-u", `${url.replace(/\/$/, "")}/FUZZ`, "-w", wl, "-mc", "200,204,301,302,307,401,403"], { timeoutMs: 30 * 60 * 1000 });
        return text(resultText("web_dir_brute", r));
      }
      const gobuster = requireCapability("gobuster");
      const r = await runToLoot("web_dir_brute", [gobuster, "dir", "-u", url, "-w", wl, "-q"], { timeoutMs: 30 * 60 * 1000 });
      return text(resultText("web_dir_brute", r));
    }
  );
  ...
  server.tool(
    ...
    async ({ url }) => {
      ...
      const r = await runToLoot("web_vuln_scan", [nikto, "-h", url], { timeoutMs: 30 * 60 * 1000 });
  ...
  server.tool(
    ...
    async ({ url, level, risk }) => {
      ...
      const r = await runToLoot(
        "web_sql_inject",
        [sqlmap, "-u", url, "--batch", "--level", level ?? "1", "--risk", risk ?? "1", "--output-dir", "/tmp/sqlmap-out"],
