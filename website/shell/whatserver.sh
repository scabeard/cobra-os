...
# Source: thc-tips-tricks-hacks-cheat-sheet-master/tools/whatserver.sh (THC, hackerschoice)
# Why vendored: cobrashell's ws() must run a LOCAL copy of this script.
# COBRA prime directive: no remote sourcing of scripts, ever. Do NOT add
# download fallbacks here — upstream provenance stays in this header and in
# shell/cobrashell.sh.
#
# COBRA changes vs upstream:
#   - The ungated https://ipinfo.io lookup (public IP / PTR) is REMOVED —
#     this script makes NO network connections. Use cobrashell's `ipinfo`
#     (xint-gated) for the public-IP check.
...
#
# Upstream usage (NOT the COBRA way — kept for provenance only):
...
# COBRA: upstream defined HTTPS_curl/HTTPS_wget/HTTPS here and used them for
# an ungated https://ipinfo.io lookup. Both are removed — this script makes
# NO network connections (use cobrashell's `ipinfo`, xint-gated, for that).
...
id
# COBRA: upstream fetched https://ipinfo.io here (ungated internet egress).
# Removed — use cobrashell's `ipinfo` (xint-gated) for the public-IP lookup.
...
echo -e "${CW}>>>>> whatserver — vendored THC tool, COBRA OS local copy (no network)${CN}"
# return with "success"
