...
#   - "Re-source me" hints point at the LOCAL copy (/etc/cobra/cobrashell.sh)
#     on the OS, and at the project's own mirror
#     (https://cobra-os.com/shell/cobrashell.sh) on any other box
...
# On any other machine (same file, mirrored on the project site):
...
#
# COBRA: on the OS the canonical copy is the LOCAL file; everywhere else the
# same file is mirrored on the project site. The hint below is all that uses
# the URL — cobrashell itself never remote-sources, never phones home.
if [ -f /etc/cobra/cobrashell.sh ]; then
    _HSURL="/etc/cobra/cobrashell.sh"
    _HS_HINT='eval "$(cat /etc/cobra/cobrashell.sh)"'
else
    _HSURL="https://cobra-os.com/shell/cobrashell.sh"
    _HS_HINT='source <(curl -fsSL https://cobra-os.com/shell/cobrashell.sh)'
fi
_HSURLORIGIN=
...
xsu() {
    ...
    [ $# -le 0 ] && echo >&2 -e "May need to cut & paste: ' ${CDC}${_HS_HINT}${CN}'"
    bak="$_HS_HOME_ORIG"
    unset _HS_HOME_ORIG
...
ssh() {
    ...
    [ -n "$_HSURL" ] && echo -e "May need to cut & paste: ' ${CDC}${_HS_HINT}${CN}'"
...
zapme() {
    ...
    HS_WARN "Starting new/zap'ed shell. Type '${CDC}${_HS_HINT}${CDM}' again."
    [ -z "$name" ] && {
...
_hs_mk_pty() {
    ...
    read -r
    echo -e ">>> Cut & paste ${CDC}${_HS_HINT}${CN}"
...
[ -z "$QUIET" ] && {
    ...
    [ -z "$_HS_HUSH" ] && {
        ...
        [ -n "$_HSURLORIGIN" ] && HS_WARN "Better use: ' ${CDC}${_HS_HINT}${CDM}'${CN}"
    }
    export _HS_HUSH=1
}
